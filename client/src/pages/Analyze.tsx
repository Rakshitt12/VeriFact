...
function EvidenceCard({ item }: { item: EvidenceView }) {
  return <article className="evidence-card"><div className="evidence-top"><span className={`source-avatar ${item.tone}`}>{item.source.slice(0, 1)}</span><span className="source-name">{item.source}</span>{item.url ? <a className="evidence-link" aria-label={`Open ${item.source}`} href={item.url} target="_blank" rel="noreferrer"><ExternalLink size={14} /></a> : <span className="evidence-link" aria-hidden="true"><ExternalLink size={14} /></span>}</div><h3>{item.url ? <a href={item.url} target="_blank" rel="noreferrer" style={{ color: "inherit", textDecoration: "none" }}>{item.title}</a> : item.title}</h3><p>{item.meta}{item.snippet ? ` — ${item.snippet.slice(0, 140)}${item.snippet.length > 140 ? "…" : ""}` : ""}</p><div className={`evidence-tag${item.contradicting ? " contra" : ""}`}><Check size={13} /> {item.tag}</div></article>;
}

function FactCheckCard({ item }: { item: FactCheckItem }) {
  return <article className="evidence-card"><div className="evidence-top"><span className="source-avatar orange">{item.fact_checker.slice(0, 1)}</span><span className="source-name">{item.fact_checker} · Fact-check</span><a className="evidence-link" aria-label={`Open fact-check by ${item.fact_checker}`} href={item.url} target="_blank" rel="noreferrer"><ExternalLink size={14} /></a></div><h3><a href={item.url} target="_blank" rel="noreferrer" style={{ color: "inherit", textDecoration: "none" }}>{item.rating_label}</a></h3><p>{item.explanation.slice(0, 160)}{item.explanation.length > 160 ? "…" : ""}</p><div className="evidence-tag contra"><Check size={13} /> Rated: {item.verdict}</div></article>;
}
...
function exportReportJson(response: VerificationResponse) {
  try {
    ...
    anchor.href = url;
    anchor.download = `verifact-report-${response.request_id}.json`;
